...
const SharePromptButtons = ({ bestieName, onClose, messageType = 'circle', messengerLink }) => {
  // Get the appropriate message based on messageType
  const getMessage = () => {
    if (messageType === 'messenger') {
      return `Hey! Can you please click this link to be added as my emergency contact on Besties? ❤️\n\n${messengerLink || window.location.origin}`;
    } else {
      // Default for circle besties
      return `Hey, can you please add a notification option to Besties so I can use you as an emergency contact? ❤️\n\nDownload Besties: ${window.location.origin}`;
    }
  };
  ...
  const handleShareToAddPhone = (platform) => {
    ...
    switch (platform) {
      case 'whatsapp':
        ...
        break;
      case 'messenger':
        const linkToShare = messageType === 'messenger' && messengerLink ? messengerLink : window.location.origin;
        if (isMobile) {
          window.location.href = `fb-messenger://share?link=${encodeURIComponent(linkToShare)}`;
          setTimeout(() => {
            window.open(`https://www.facebook.com/dialog/send?link=${encodeURIComponent(linkToShare)}&app_id=&redirect_uri=${encodeURIComponent(window.location.origin)}`, '_blank');
          }, 1500);
        } else {
          window.open(`https://www.facebook.com/dialog/send?link=${encodeURIComponent(linkToShare)}&app_id=&redirect_uri=${encodeURIComponent(window.location.origin)}`, '_blank', 'width=600,height=400');
        }
        break;
      case 'instagram':
  ...
  return (
    <div className="mt-2 p-4 bg-purple-50 dark:bg-purple-900/30 border-2 border-purple-200 dark:border-purple-800 rounded-xl relative">
      <div className="flex items-center justify-between mb-3">
        <p className="text-sm font-semibold text-purple-900 dark:text-purple-200">
          📱 {messageType === 'messenger' 
            ? `Ask ${bestieName} to click the link to be added as emergency contact`
            : `Ask ${bestieName} to add notification option`}
        </p>
        {onClose && (
          <button
            type="button"
      ...
      <div className="grid grid-cols-3 gap-2">
        <button
          ...
          className="flex flex-col items-center gap-1 p-3 rounded-lg bg-[#E91E63] hover:bg-[#C2185B] text-white transition-colors"
        >
