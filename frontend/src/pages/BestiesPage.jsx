...
const BestiesPage = () => {
  ...
  useEffect(() => {
    ...
    const loadActivityFeed = async () => {
      ...
      // OPTIMIZED: Batch all queries instead of sequential per-bestie queries
      // This reduces from 5 queries per bestie to ~3-4 total queries
      ...
      for (let i = 0; i < bestieIds.length; i += BATCH_SIZE) {
        ...
        // Query 2: Check-ins where current user is selected bestie (using denormalized bestieUserIds)
        ...
        // Query 3: Posts from these besties
      ...
      queryResults.forEach(result => {
        if (result.type === 'checkins' || result.type === 'checkinsAsBestie') {
          result.snap.forEach((checkInDoc) => {
            ...
            // Skip if not from our besties (for checkInsAsBestie query)
      ...
      // Process request attention and badges from batched data
